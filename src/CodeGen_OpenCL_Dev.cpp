...
void CodeGen_OpenCL_Dev::CodeGen_OpenCL_C::visit(const For *loop) {
    ...
    if (is_gpu_var(loop->name)) {
        ...
        if(loop->annotations.size() != 0){
            stream << get_indent() << "/*@\n";
            indent++;
            AnnotationPrinter ap(stream, false, false, buffer_types);
...
void CodeGen_OpenCL_Dev::CodeGen_OpenCL_C::visit(const Store *op) {
    ...
    if (emit_atomic_stores) {
        ...
        Expr equiv_load = Load::make(t, op->name, op->index, Buffer<>(), op->param, op->predicate, op->alignment, op->lemma);
...
void CodeGen_OpenCL_Dev::CodeGen_OpenCL_C::add_kernel(Stmt s,
    ...
    AnnotationPrinter ap(stream, false, true, buffer_types);
