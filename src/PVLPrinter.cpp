...
pure rational abs(rational x) = x >= 0 ? x : -x;
 ...
 decreases;
 ensures !is_int(x) ==> rational_to_int(\result) == rational_to_int(x) + 1;
pure rational ceil_f32(rational x) = is_int(x) ? x : int_to_rational(rational_to_int(x) + 1);

 decreases;
pure rational ceil_f64(rational x) = ceil_f32(x);

 decreases;
 ensures !is_int(x) ==> rational_to_int(\result) == rational_to_int(x);
pure rational floor_f32(rational x) = is_int(x) ? x : int_to_rational(rational_to_int(x));

 decreases;
pure rational floor_f64(rational x) = floor_f32(x);
...
pure rational fast_inverse_f32(rational x) = div_rat(1.0f, x);
 ...
 decreases;
 ensures !is_int(x) ==> rational_to_int(\result) == rational_to_int(x + 0.5f);
pure rational round_f32(rational x) = is_int(x) ? x : int_to_rational(rational_to_int(x + 0.5f));

 decreases;
pure rational round_f64(rational x) = round_f32(x);
