    ...
    if(is_pvl()){
        if(const Call *c = op->value.as<Call>()){
            ...
            if(c->name.compare(Call::buffer_get_host) == 0){
                ...
                stream << get_indent() << "int[] " << print_name(op->name) << " = " 
...
void CodeGen_C::visit(const For *op) {
    ...
    if(is_pvl() && op->annotations.size() != 0){
        indent++;
        AnnotationPrinter ap(stream);
