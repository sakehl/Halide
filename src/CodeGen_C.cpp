...
const string old_globals = R"INLINE_CODE(
extern "C" {
...
string buffer_header(string type){
...
string buffer_annotations(string buffer_name, Type t, int dimensions, Indentation indent, bool is_pvl, bool is_const){
    ...
    //     if(i > 0)
    //         min_val_s << " + ";
    //     min_val_s << buffer_name << ".min_" << i << " * abs(" << buffer_name << ".stride_" << i << ")";
    // }
...
CodeGen_C::CodeGen_C(ostream &s, const Target &t, OutputKind output_kind, const std::string &guard, bool const_unique_buffers)
    ...
    if(is_pvl()){
        stream << pvl_header;
        stream << buffer_header("int");
        stream << buffer_header("rational");
        return;
    }
...
string get_simple_type(Type t){
    if(t.is_int_or_uint()) return "int";
    if(t.is_float()) return "float";
    internal_error << "Type " << t << " is not supported for HaliVer Verification";
    return "";
}
...
bool compare_halide_type_codes(halide_type_code_t t1, halide_type_code_t t2){
    if(t1 == t2){
        return true;
    }
    if(t1 == halide_type_code_t::halide_type_int && t2 == halide_type_code_t::halide_type_uint){
        return true;
    }
    if(t1 == halide_type_code_t::halide_type_uint && t2 == halide_type_code_t::halide_type_int){
        return true;
    }
    return false;
}
...
void CodeGen_C::visit(const Max *op) {
    ...
    if (op->type.is_scalar()) {
        ostringstream rhs;
        rhs << print_expr(op->a) << ">" << print_expr(op->b) << " ? " << print_expr(op->a) << " : " << print_expr(op->b);
        print_assignment(op->type, rhs.str());
        // print_expr(Call::make(op->type, "::halide_cpp_max", {op->a, op->b}, Call::Extern));
    } else {
        internal_error << "Unsupported max by HaliVer";
        ostringstream rhs;
...
void CodeGen_C::visit(const Min *op) {
    ...
    if (op->type.is_scalar()) {
        ...
        internal_error << "Unsupported min by HaliVer";
        ostringstream rhs;
...
void CodeGen_C::visit_relop(Type t, const Expr &a, const Expr &b, const char *scalar_op, const char *vector_op) {
    if (t.is_scalar() || !using_vector_typedefs) {
        ...
        internal_error << "Unsupported relop by HaliVer";
...
void CodeGen_C::visit(const Select *op) {
    ...
    if (op->condition.type().is_scalar()) {
        if(!(op->true_value.type() == op->false_value.type() && op->true_value.type() == op->type)){
            rhs << "(" << type << ")";
        }
        rhs << "(" << cond
            << " ? " << true_val
    ...
    if(is_pvl()){
        if(const Call *c = op->value.as<Call>()){
            if(c->name.compare(Call::buffer_get_host) == 0){
                ...
                vector<string> args(c->args.size());
...
void CodeGen_C::visit(const For *op) {
    ...
    if (op->for_type == ForType::Parallel) {
        if(!is_pvl()){
            ...
            stream << get_indent() << "par threads(int " << print_name(op->name) << " = "
                << id_min << " .. " << "(" << id_min << " + " << id_extent << "))\n";
        }
    } else {
    ...
    if(op->for_type == ForType::Serial || !is_pvl()){
...
void CodeGen_C::visit(const Allocate *op) {
    ...
    if(is_pvl()){
        ...
        if(op->extents.size() == 0)
            ...
            size_id = print_expr(op->extents[0]);
        ...
        stream << "[] " << op_name << " = new " << type_no_space << "[" << size_id << "];\n";
...
void CodeGen_C::print_heap_free(const std::string &alloc_name) {
    if (heap_allocations.contains(alloc_name)) {
        ...
        stream << get_indent() << "free("<< print_name(alloc_name) << ");\n";
...
void CodeGen_C::visit(const Evaluate *op) {
    ...
    if(const Call *c = op->value.as<Call>()){
        // Do not print this in PVL
        if(c->name.compare(Call::buffer_set_host_dirty) == 0) return;
    }
...
void CodeGen_C::visit(const Shuffle *op) {
    internal_error << "Unsupported Shuffle by HaliVer";
...
void AnnotationPrinter::visit(const Variable *op) {
    ...
    } else if (ends_with(op->name, ".buffer.host")) {
        output = op->name;
        output.erase(output.length() - string(".buffer.host").length());
        if(is_pvl){
            output = c_print_name(output) + "_buffer.host";
        } else {
            output = c_print_name(output) + "_buffer->host";
        }
    } else {
...
void AnnotationPrinter::visit(const Load *op) {
    ...
    //     stream << "(" << op->type << ")";
    // }
    if (ends_with(op->name, ".buffer.host")) {
        string output = op->name;
        output.erase(output.length() - string(".buffer.host").length());
        stream << c_print_name(output);
        if(is_pvl){
             stream << "_buffer.host";
        } else {
            stream << "_buffer->host";
...
void AnnotationPrinter::visit(const FloatImm *op) {
        stream << std::to_string(op->value);
}
...
void AnnotationPrinter::visit(const Cast *op){
    ...
    internal_assert(!e.type().is_vector()) << "Vector types are not supported by HaliVer";
    ...
    if(is_pvl){
        ...
        if(t.is_float()){
            ...
            internal_error << "Not supported cast by C";
        }
        stream << ")";
        print(e);
    }
