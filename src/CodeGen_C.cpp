...
string buffer_annotations(string buffer_name, int dimensions, Indentation indent = Indentation{0}){
    std::ostringstream o;
...
void CodeGen_C::visit(const Div *op) {
    int bits;
    if (!is_pvl() && is_const_power_of_two_integer(op->b, &bits)) {
...
void CodeGen_C::visit(const For *op) {
    ...
    if(is_pvl()){
        ...
        indent++;
        AnnotationPrinter ap(stream);
        stream << get_indent() << (op->for_type == ForType::Parallel ? "context " : "loop_invariant ") 
            << id_min << " <= " << print_name(op->name) 
            << " && " << print_name(op->name);
        stream << (op->for_type == ForType::Parallel ? " < " : " <= ");
        stream << id_min << " + " << id_extent << ";\n";
    ...
    if(!(op->for_type == ForType::Parallel && is_pvl())){
...
void AnnotationPrinter::visit(const Load *op) {
    ...
    if (ends_with(op->name, ".buffer.host")) {
        ...
        output.erase(output.length() - string(".buffer.host").length());
        stream << c_print_name(output) << "_buffer.host";
    } else {
